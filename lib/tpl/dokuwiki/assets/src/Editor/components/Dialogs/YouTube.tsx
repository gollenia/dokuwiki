...
const YouTube = (props: Props) => {
    ...
    const extractUrlInfo = (url: string) => {
        ...
        if (match[7].length != 11) {
            setError(
                'Youtube ID ist zu ' + (match[7].length > 11 ? 'lang' : 'kurz') + '. Die ID hat genau 11 Zeichen.'
            );
            resetVideoObject();
            return;
        }
    ...
    return (
        <>
            <div
                className={'modal modal-md fade ' + (showYouTube ? 'show' : '')}
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="container">
                            <div>
                                <input
                                    ...
                                    type="text"
                                    value={videoObject.url}
                                    onChange={event => extractUrlInfo(event.target.value)}
                                />
                            ...
                            <div className="form-check form-switch mt-4">
                                <input
                                    ...
                                    onChange={event => {
                                        setVideoObject(video => {
                                            return { ...video, autoplay: !videoObject.autoplay };
                                        });
                                    }}
                                />
                        ...
                        <div className="modal-footer">
                            <button className="btn btn-secondary" onClick={() => setShowYouTube(false)}>
                                Abbrechen
                            </button>
                            <button disabled={!videoObject.valid} className="btn btn-primary" onClick={() => clickOk()}>
                                OK
                            </button>
                        </div>
                    </div>
