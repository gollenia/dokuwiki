...
const Combobox: React.FC<ComboboxProps> = (props) => {
	...
	return (
		<div className='combobox' onKeyDown={(event) => keyPress(event)}>
			<input className="form-control" ref={input} type="text" onMouseOver={() => { setListSelect(-1) }} onClick={(event) => { }} placeholder={selection != -1 ? options[selection].label : placeholder} value={inputField} onChange={(event) => setInputField(event.target.value)} />
			<ul>
