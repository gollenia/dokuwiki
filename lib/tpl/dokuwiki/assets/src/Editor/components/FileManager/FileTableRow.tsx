...
const FileTableRow = (props: Props) => {
	...
	return (
		<tr className='filetable-row'>
			<td><FileIcon className="filetable-icon"  extension={file.extension} size={32}/></td>
			<td>
				...
				{ renameMode && <input 
					...
					value={fileName} 
					onKeyDown={(event) => {renameFile(event)} } 
					className={'form-control form-control-sm ' + (!renameMode ? 'invisible' : '')}
				/> }
			</td>
			<td><span className="filetable-size"><FormattedNumber value={size.value} unit={size.unit} style="unit" maximumSignificantDigits={3} /></span></td>
			<td>{file.count}</td>
			<td>
