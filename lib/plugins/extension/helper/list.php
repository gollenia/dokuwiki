...
class helper_plugin_extension_list extends DokuWiki_Plugin
{
	...
	{
		$this->form .= '<div class="row row-cols-1 row-cols-md-3 g-4">';
	}
	...
	public function addRow(helper_plugin_extension_extension $extension, $showinfo = false)
	{
		...
		$this->form .= $this->makeScreenshot($extension);
		$this->form .= $this->makeLegend($extension, $showinfo);
		$this->form .= $this->makeActions($extension);
		$this->endRow();
	}
	...
	{
		global $lang;
		$this->form .= '<div class="notfound">' . $lang['nothingfound'] . '</div>';
	}
	...
	{
		$this->form .= '<div class="col"><div id="extensionplugin__' . hsc($extension->getID()) .
			'" class="card">';
	}
	...
	public function makeScreenshot(helper_plugin_extension_extension $extension)
	{
		...
		if ($screen) {
			...
			$img = '<img alt="' . $title . '" width="600" class="card-img-top" src="' . hsc($thumb) . '" />';
		} elseif ($extension->isTemplate()) {
			$img = '<img alt="" class="card-img-top" width="600" src="' . DOKU_BASE .
	...
	public function makeLinkbar(helper_plugin_extension_extension $extension)
	{
		...
		if ($extension->getTags()) {
			...
			foreach ($extension->getTags() as $tag) {
				...
				$html .= '<a class="badge bg-secondary" href="' . $url . '">' . hsc($tag) . '</a>';
			}
			$html .= '</span>';
		}
		$html .= '</div>' . DOKU_LF;
	...
	public function makeActions(helper_plugin_extension_extension $extension)
	{
		...
		if ($extension->isInstalled()) {
			if (($canmod = $extension->canModify()) === true) {
				...
				$errors .= '<div class="card-footer">' . $this->getLang($canmod) . '</div>';
			}
			...
			if ($extension->isGitControlled()) {
				$errors .= '<div class="card-footer"><small>' . $this->getLang('git') . '</small></div>';
			}
			...
			if (
				...
				$errors .= '<div class="card-footer"><small>' . $this->getLang('auth') . '</small></div>';
			}
		} else {
			if (($canmod = $extension->canModify()) === true) {
				if ($extension->getDownloadURL()) {
				...
				$errors .= '<div class="card-footer"><small>' . $this->getLang($canmod) . '</small></div>';
			}
		}
	...
	public function makeAction($action, $extension)
	{
		...
		$classes = ['uninstall' => 'danger', 'install' => 'primary', 'reinstall' => "warning", 'disable' => 'warning'];

		$classes = 'btn btn-' . $classes[$action];
