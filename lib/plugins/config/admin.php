...
class admin_plugin_config extends AdminPlugin
{
	...
	public function html()
	{
		...
		if (!$this->configuration->isLocked()) {
			echo '<p class="d-flex flex-row-reverse gap-4">';
			echo '<input type="hidden" name="save"   value="1" />';
			echo '<button type="submit" class="btn btn-primary mr-1" name="submit" accesskey="s">' . $lang['btn_save'] . '</button>';
			echo '<button type="reset" class="btn btn-outline-danger">' . $lang['btn_reset'] . '</button>';
			echo '</p>';
		}
		...
		foreach ($this->configuration->getSettings() as $setting) {
			if (is_a($setting, SettingHidden::class)) {
				...
				echo '<h2 class="mb-5">' . $setting->prompt($this) . '</h2>';
				echo '<div class="">';
			} else {
				// config settings
				...
				echo '<div class="g-col-4">';


				echo $icon . $label;
				echo '</div>';

				echo '<div class="g-col-5"' . $error . '>' . $input . '</div>';
				echo '</div>';
			}
		}
